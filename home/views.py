...
def set_search(request):
    if request.method == 'POST':
        try:
            print("body", request.body)
            data = json.loads(request.body)
            search_state = data.get('searching', False)  # Default to False if not provided
            
            json_data = '{"searching":' + ('true' if search_state else 'false') + '}'
            
            headers = {'Content-Type': 'application/json'}
            print("json_data", json_data)
            response = requests.post(f'{ESP32_IP}/set_search', data=json_data, headers=headers)
            print("====", response.text)
            if response.status_code == 200:
                return JsonResponse({'status': 'success', 'message': 'Starting searching mode!'})
            else:
                return JsonResponse({
                    'status': 'error',
                    'message': 'Failed to turn on searching mode on ESP32.',
                    'details': response.text  # Add the response text from the ESP32 for more info
                })
